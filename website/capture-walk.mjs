...
// Scoped to the nav strip and picking the leaf-most match: the Assistant pill wraps an icon plus a
// text node (childElementCount 1, not 0), so a bare "no children" filter clicks nothing for it, and
// an unscoped text match can hit the word elsewhere on the page (a doc body, a task title). The
// Review pill also carries a pending-count badge appended to its text ("Review3"), stripped by
// `norm` below - without it this matched nothing and silently left the shot on the prior tab.
