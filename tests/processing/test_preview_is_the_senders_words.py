...
Every mail from outside MFA opens with a security banner, so the Timeline's Message step read "This
email was sent from outside of MFA. ** Do not click links or download attachments..." on every one
...
BANNER = ('This email was sent from outside of MFA. ** Do not click links or download attachments '
          'unless you know the content is safe. If you are not sure whether this email is safe, '
          'contact the MFA Helpdesk at 866-MFA-DESK.**')
...
def a_message(db, subject, body, sender='notifications@github.com'):
    mid = db.add_message({'ExternalId': f'x:{subject}', 'ConversationId': f'c:{subject}',
                          'Channel': 'email', 'Subject': subject, 'FromName': 'Uri',
...
def test_the_external_banner_is_not_the_message(db):
    ...
    assert 'sent from outside of MFA' not in preview, 'the wrapper is not the message'
...
def test_an_ordinary_body_is_untouched(db):
    ...
    mid = a_message(db, 'Refund', body, sender='dvora@example.test')
